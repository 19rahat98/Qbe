...
class PlaceBloc extends ChangeNotifier{
  ...
  Future getData() async {
    QuerySnapshot snap = await Firestore.instance.collection('places').getDocuments();
  ...
  Future getLovesAmount(timestamp) async {
    final DocumentReference ref = Firestore.instance.collection('places').document(timestamp);
    DocumentSnapshot snap = await ref.get();
  ...
  Future commentsIncrement (timestamp) async{
    Firestore.instance.collection('places').document(timestamp).updateData({
