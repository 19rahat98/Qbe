    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          Expanded(
              child: FutureBuilder(
              future: widget.title == 'User Reviews' ? cb.getData(widget.timestamp, 'places') : cb.getData(widget.timestamp,'blogs'),
              builder: (BuildContext context, AsyncSnapshot<List> snap){

                if(snap.connectionState == ConnectionState.none) return emptyPage(Icons.signal_wifi_off, 'No Internet');
                else{
                  if(!snap.hasData) return Center(child: CircularProgressIndicator());
                  if(snap.hasError) return emptyPage(Icons.error_outline, 'Something is wrong');
                  if(snap.data.isEmpty) return emptyPage(Icons.mode_comment, 'No comment found!');
