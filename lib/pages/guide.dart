...
class GuidePage extends StatefulWidget {
  ...
  final double lat;
  final double lng;
  GuidePage({Key key,@required this.timestamp, this.lat, this.lng}) : super(key: key);
...
class _GuidePageState extends State<GuidePage> {
  ...
  Future getData() async {
   await Firestore.instance.collection('places').document(widget.timestamp).collection('travel guide').document(widget.timestamp).get().then((DocumentSnapshot snap) {
      setState(() {
        data = snap.data;
      });
  ...
  getPolyLineData ()async{
    GoogleMapPolyline googleMapPolyline = new  GoogleMapPolyline(apiKey: Config().mapAPIKey );
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
            Text('Steps', style: TextStyle(fontFamily: 'Poppins', fontWeight: FontWeight.w600, fontSize: 18),),
            Container(
                    margin: EdgeInsets.only(top: 8, bottom: 8),
                    height: 3,
                    width: 70,
              ...
              return Padding(
                ...
                child: Row(
                  children: <Widget>[
                    Column(
                      children: <Widget>[
                        CircleAvatar(radius: 15, child: Text('${index + 1}'),backgroundColor: GuideColors().colors[index]),
                        Container(
                          height: 90,
                          width: 2,
